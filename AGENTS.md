...
- **Version lives in FOUR places — keep them in sync:** `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, and the `APP_VERSION` fallback in `BackupFeature.tsx` (runtime value comes from `getVersion()`). Releasing = bump all four → tag `vX.Y.Z` → push the tag; CI builds, signs, and publishes.
