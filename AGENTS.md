...
The Rust storage module (`src-tauri/src/storage.rs`) owns the workspace paths, migration, filename sanitization, atomic-write contract, and exposes three commands; `src-tauri/src/lib.rs` only assembles Tauri:
...
The app self-updates via `tauri-plugin-updater` + `tauri-plugin-process`. Four places cooperate: **`src/App.tsx`** drives the UX (`checkForUpdate` / `runUpdate` / `dismissUpdate`, the fixed `.v2-update-banner`, and the Backup page "关于与更新" card), **`src-tauri/src/lib.rs`** registers the plugins, **`src-tauri/src/system_proxy.rs`** bridges the system proxy, and **`.github/workflows/release-windows.yml`** signs and publishes the manifest.
